...
__global__ void Marlin(
  ...
  int iters = ceildiv_m(k_tiles * n_tiles * parallel, gridDim.x);
  ...
  if (group_blocks != -1)
    iters = (group_blocks / thread_k_blocks) * ceildiv_m(iters, (group_blocks / thread_k_blocks));
  ...
  auto init_slice = [&] () {
    ...
    slice_count = 1;
    slice_idx = 0;
    int col_first = iters * ceildiv_m(k_tiles * slice_col_par, iters);
    if (col_first <= k_tiles * (slice_col_par + 1)) {
      int col_off = col_first - k_tiles * slice_col_par;
      slice_count = ceildiv_m(k_tiles - col_off, iters);
      if (col_off > 0)
        slice_count++;
  ...
  constexpr int a_sh_wr_iters = ceildiv_m(a_sh_stage, a_sh_wr_delta); // number of shared write iterations for a tile
  ...
  auto fetch_to_shared = [&] (int pipe, int a_off, bool pred = true) {
    if (pred) {
      ...
      if (group_blocks != -1 && pipe % ((group_blocks == -1 ? 1024 : group_blocks) / thread_k_blocks) == 0) {
  ...
  auto fetch_to_registers = [&] (int k, int pipe) {
    ...
    if (group_blocks != -1) {
      int4* sh_s_stage = sh_s + s_sh_stage * ((group_blocks / thread_k_blocks) * (pipe / ((group_blocks == -1 ? 1024 : group_blocks) / thread_k_blocks)));
  ...
  auto write_result = [&] () {
    ...
    #pragma unroll
    for (int i = 0; i < ceildiv_m(16 * thread_m_blocks, threads / (2 * thread_n_blocks)); i++) {
...
int marlin_cuda(
  ...
  int tot_m = prob_m;
  int tot_m_blocks = ceildiv_m(tot_m, 16);
...
extern "C" int launch_marlin(
  ...
  int max_par
    ...
    return marlin_cuda(A, B, C, s, prob_m, prob_n, prob_k, workspace, groupsize, dev, stream, thread_k, thread_n, sms, max_par);
}
