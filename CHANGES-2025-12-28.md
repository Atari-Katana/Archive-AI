...
- Brain now uses `config.VORPAL_MODEL` everywhere (chat, agents, verification, memory worker) to avoid 404s when the served model changes; AWQ override also sets `VORPAL_MODEL` for brain. Memory worker shutdown uses `aclose` to avoid errors.
